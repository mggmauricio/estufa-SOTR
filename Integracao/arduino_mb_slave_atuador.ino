...
enum {
        MB_SLAVE = 2,	/* modbus slave id */ //ENDEREÇO MODBUS do SLAVE
...
void loop() 
{
  ...
  Serial.print(regs[MB_REG0]);
  Serial.print("  ");
  Serial.print(elapsedTime);
  Serial.print("  ");
  if (elapsedTime < timeout) {
    digitalWrite(2,1);
    analogWrite(3,regs[MB_REG0]);
...
/* global variables */
unsigned int Txenpin = 6;        /* Enable transmission pin, used on RS485 networks */
...
int receive_request(unsigned char *received_string) 
{
        ...
        /* FIXME: does hc12.available wait 1.5T or 3.5T before exiting the loop? */
